...
class SMSRequestSerializer(serializers.ModelSerializer):
    SmsSid = serializers.CharField(
        max_length=34, required=True, source="sms_sid")
    AccountSid = serializers.CharField(
        max_length=34, required=True, source="account_sid")

    From = serializers.CharField(
        max_length=30, required=True, source="from_number")
    FromCity = serializers.CharField(
        max_length=30, required=False, source="from_city")
    FromState = serializers.CharField(
        max_length=30, required=False, source="from_state")
    FromZip = serializers.CharField(
        max_length=30, required=False, source="from_zip")
    FromCountry = serializers.CharField(
        max_length=120, required=False, source="from_country")

    To = serializers.CharField(
        max_length=30, required=True, source="to_number")
...
class SMSStatusSerializer(serializers.ModelSerializer):
    ...
    SmsStatus = serializers.CharField(
        max_length=30, required=True, source="status")
